...
export function chooseBcdCandidate(chain) {
  ...
  const netDebit = Number((buyPrice(longLeg) - sellPrice(shortLeg)).toFixed(2));
...
export function buildBcdPaperPosition(candidate, openedAt = new Date().toISOString()) {
  return {
    ...
    opened_at: openedAt,
    legs: [
      { strike: candidate.longLeg.strike, type: 'Call', side: 'long', expiration: candidate.longLeg.expiration },
      { strike: candidate.shortLeg.strike, type: 'Call', side: 'short', expiration: candidate.shortLeg.expiration },
    ],
  };
}
