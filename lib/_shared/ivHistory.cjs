...
async function getIVRank(ticker) {
    ...
    try {
        ...
        // IV Momentum: 5-trading-day change in IV30 (rows are sorted desc so index 5 = ~5d ago)
        const iv5dAgo = iv30Values.length >= 6 ? iv30Values[5] : null;
        const iv5dChange = iv5dAgo != null ? Number((currentIv30 - iv5dAgo).toFixed(2)) : null;
        const ivTrend = iv5dChange == null ? null
            : iv5dChange > 2 ? 'rising'
            : iv5dChange < -2 ? 'falling'
            : 'flat';
