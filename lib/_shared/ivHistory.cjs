...
/** Track which tickers have been backfilled this process to avoid repeated Tiingo calls. */
...
// =============================================================================
// INLINE RV BACKFILL — fetches Tiingo daily candles, computes RV30, upserts
// =============================================================================

/**
 * Fetch daily candles from Tiingo REST API (split+dividend adjusted).
...
async function _fetchDailyCandles(ticker, fromDate, toDate) {
    ...
    if (!token) {
        console.warn('[IVHistory backfill] TIINGO_API_TOKEN not set — skipping backfill');
        return [];
    }

    const url = `https://api.tiingo.com/tiingo/daily/${ticker.toUpperCase()}/prices?startDate=${fromDate}&endDate=${toDate}&token=${token}`;
    try {
        const res = await fetch(url, {
            headers: { 'Content-Type': 'application/json' },
        });
        if (!res.ok) {
            console.warn(`[IVHistory backfill] Tiingo candles ${res.status} for ${ticker}`);
            return [];
        }
        ...
        console.warn(`[IVHistory backfill] Tiingo fetch error for ${ticker}:`, e.message);
        return [];
    }
}
...
/**
 * Inline backfill: fetch ~500 days of Tiingo candles → compute RV30 → upsert as rv_proxy.
 * Called automatically by getIVRank when total samples < MIN_BACKFILL_THRESHOLD.
 * Skips if already backfilled this process or if Tiingo token is missing.
 */
 ...
 * If total samples < MIN_BACKFILL_THRESHOLD, auto-backfills RV proxy from Tiingo
...
async function getIVRank(ticker) {
    ...
    async function _computeRank() {
        ...
        if (useLive) {
            ...
            // Use pure RV proxy series — don't mix live IV into RV window.
            // Live IV and HV×1.15 are different scales; mixing inflates rank to 100%.
            iv30Values = rvIvValues;
