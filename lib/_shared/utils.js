 ...
 * @param {string} exp YYYY-MM-DD or date string
 * @returns {boolean}
 */
export function isOptionExpired(exp) {
    if (!exp || typeof exp !== 'string') return false;
    const normalized = normalizeExpiration(exp);
    const expDate = (normalized || '').slice(0, 10);
