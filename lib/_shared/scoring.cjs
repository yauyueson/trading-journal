...
const LOQ_WEIGHTS = { lambda: 0.30, gammaEff: 0.20, gammaThetaRatio: 0.15, thetaBurn: 0, deltaBonus: 0.15, breakevenPenalty: 0.15 };
const LOQ_DT_WEIGHTS = { lambda: 0.30, gammaEff: 0.35, gammaThetaRatio: 0.20, thetaBurn: 0, breakevenPenalty: 0.05, penaltyMult: 0.2 };
...
const buildLOQFactors = (zLambda, zGammaEff, zThetaBurn, ivAdjustment, deltaBonus, thetaBurn, zGT, bePenalty, dte, zVegaEff) => {
...
const parseChain = (options, currentPrice, targetDTE = null) => {
    ...
    for (const opt of options) {
        ...
        // A real IV > 2.0 (200%) is extremely rare and almost always a unit error.
        if (iv > 2.0) iv = iv / 100;
...
const calculateUnifiedScore = (candidate, strategyCategory, regimeMode, ivRvRatio, opts = null) => {
    ...
    // Pine Script setup-aware weighting: if setup provided, regime already validated → shift 5% from EV to Regime
    const hasPineSetup = opts && opts.setup && opts.setup !== '' && opts.setup !== 'Mixed' && opts.setup !== 'Other';
    const wEV = hasPineSetup ? 0.35 : 0.40;
    const wPOP = 0.20;
    const wRegime = hasPineSetup ? 0.30 : 0.25;
    const wLiq = 0.15;
