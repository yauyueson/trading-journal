...
/** Endpoint-aware cache TTL: candle data (historical, immutable intra-minute) gets 5 min; snapshots stay at 60s. */
function getCacheTTL(endpoint) {
    if (endpoint.startsWith('/v2/aggs/')) return 5 * 60 * 1000; // 5 min for candles
                ...
                if ((response.status === 429 || response.status >= 500) && attempt < MAX_RETRIES) {
                    ...
                    if (response.status === 429) {
                        ...
                        if (retryAfter) {
                            ...
                            if (!isNaN(retryMs) && retryMs > 0 && retryMs <= 5000) delay = retryMs;
                        }
                    }
