    ...
    _rlChain = _rlChain.then(async () => {
        ...
        if (_rlTimestamps.length >= rpm) {
            ...
            if (waitMs > 9000) {
                ...
                throw new Error(`[Polygon] Rate limit: ${_rlTimestamps.length}/${rpm} RPM — would wait ${Math.ceil(waitMs / 1000)}s (set POLYGON_RATE_LIMIT_RPM to match your plan)`);
            }
...
async function fetchPolygon(endpoint, params = {}, useCache = true) {
    ...
    if (useCache && cache.has(cacheKey)) {
        ...
        if (Date.now() - timestamp < getCacheTTL(endpoint)) {
            // #region agent log
            fetch('http://127.0.0.1:7243/ingest/249f49d0-da45-4d0c-8b61-633380f5c648',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'9e09cd'},body:JSON.stringify({sessionId:'9e09cd',location:'polygon-client.js:fetchPolygon',message:'cache hit',data:{endpoint:endpoint.slice(0,80),cacheKeyLen:cacheKey.length},timestamp:Date.now(),hypothesisId:'H2'})}).catch(()=>{});
            // #endregion
            return data;
        } else {
    ...
    const promise = (async () => {
        ...
        // #region agent log
        const _rpm = parseInt(process.env.POLYGON_RATE_LIMIT_RPM || '5');
        fetch('http://127.0.0.1:7243/ingest/249f49d0-da45-4d0c-8b61-633380f5c648',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'9e09cd'},body:JSON.stringify({sessionId:'9e09cd',location:'polygon-client.js:fetchPolygon',message:'real request after rate slot',data:{endpoint:endpoint.slice(0,80),POLYGON_RATE_LIMIT_RPM:_rpm},timestamp:Date.now(),hypothesisId:'H1'})}).catch(()=>{});
        // #endregion
            ...
            try {
                ...
                if ((response.status === 429 || response.status >= 500) && attempt < MAX_RETRIES) {
                    ...
                    if (response.status === 429) {
                        // #region agent log
                        fetch('http://127.0.0.1:7243/ingest/249f49d0-da45-4d0c-8b61-633380f5c648',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'9e09cd'},body:JSON.stringify({sessionId:'9e09cd',location:'polygon-client.js:fetchPolygon',message:'429 from Polygon',data:{endpoint:endpoint.slice(0,80),attempt,RPM:parseInt(process.env.POLYGON_RATE_LIMIT_RPM||'5')},timestamp:Date.now(),hypothesisId:'H1'})}).catch(()=>{});
                        // #endregion
