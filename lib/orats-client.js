...
function normalizeORATSStrike(row) {
    ...
    // Prefer ORATS smoothed vol (smvVol) over noisy mid-market IV for scoring.
    // smvVol is a fitted surface value — eliminates bid/ask bounce noise.
    // Fall back to midIv when smvVol is not available.
    const callSmvVol = row.callSmvVol ?? null;
    const putSmvVol = row.putSmvVol ?? null;
    ...
    results.push({
        ...
        iv: callSmvVol || row.callMidIv || 0, // Prefer smoothed vol for scoring
        midIv: row.callMidIv ?? 0,             // Keep raw mid-IV for reference
        delta: callDelta,
        gamma,
        theta,
        vega,
    ...
    results.push({
        ...
        iv: putSmvVol || row.putMidIv || 0,   // Prefer smoothed vol for scoring
        midIv: row.putMidIv ?? 0,              // Keep raw mid-IV for reference
        delta: putDelta,
        gamma,
        theta,
        vega,
