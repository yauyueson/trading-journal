...
// Replaces polygon-client.js for all options data.
...
// ── Exported functions (same signatures as polygon-client.js) ──────────────────
...
/**
 * Fetch option chain with filters. Same signature as polygon-client.js.
 * @param {string} ticker
 ...
 * Same logic as polygon-client.js — reads quoteUpdatedMs from normalized chain.
 */
...
/**
 * Get pre-computed IV rank and percentile from ORATS.
 * @param {string} ticker
 * @returns {Promise<{iv, ivRank1m, ivRank1y, ivPct1m, ivPct1y}|null>}
 */
export async function getIVRank(ticker) {
    try {
        const data = await fetchORATS('/ivrank', { ticker: ticker.toUpperCase() });
        if (data.data && data.data.length > 0) return data.data[0];
    } catch (e) {
        console.warn('[ORATS] getIVRank failed:', e?.message);
    }
    return null;
}

/**
 * Get IV summary (term structure, implied move, contango, skew).
 * @param {string} ticker
 * @returns {Promise<object|null>}
 */
export async function getSummary(ticker) {
    try {
        const data = await fetchORATS('/summaries', { ticker: ticker.toUpperCase() });
        if (data.data && data.data.length > 0) return data.data[0];
    } catch (e) {
        console.warn('[ORATS] getSummary failed:', e?.message);
    }
    return null;
}
