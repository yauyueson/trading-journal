...
export function rsi(values, period) {
    ...
    // Collect `period` valid diffs to seed avgGain/avgLoss, skipping NaN pairs
    let seedGains = 0;
    let seedLosses = 0;
    let validDiffs = 0;
    let seedEndIdx = -1;
    ...
    for (let i = seedEndIdx + 1; i < len; i++) {
        ...
        if (!isFinite(curr) || !isFinite(prev)) {
            // NaN bar: decay averages (no new information)
 ...
 */
export function emaFullSeries(values, period) {
    const result = [];
    if (values.length === 0) return result;
    const k = 2 / (period + 1);
...
/**
 * T3 Smoothing (full-series NaN propagation to match Pine)
 * @param {number[]} values 
 * @param {number} period 
 * @param {number} b 
 * @returns {number[]}
 ...
 * @param {number[]} mb_o EMA(open), mb_h, mb_l, mb_c
 * @returns {{ haOpens: number[], haCloses: number[] }}
 */
export function heikinAshiPine(mb_o, mb_h, mb_l, mb_c) {
    const n = mb_o.length;
    const haOpens = [];
    const haCloses = [];
    let xhaopenPrev, haclosePrev;
    for (let i = 0; i < n; i++) {
