...
import { ema, emaFullSeries, rsi, t3_smooth, heikinAshiPine } from './indicators.js';
...
export function calculateTechScore(candles, options = {}) {
    ...
    // emaFullSeries used here (not ema) because haOpens/haCloses have leading NaNs from the
    // first EMA chain. emaFullSeries seeds from first non-NaN value, matching Pine ta.ema().
    ...
    sc_ema += 5 + Math.min(stackStrength / 0.5, 1) * 20;
    ...
    // Momentum — continuous interpolation
    let sc_mom = 50.0;
    const absCh1 = Math.abs(ch1);
    const absCh3 = Math.abs(ch3);

    sc_mom += 5 + Math.min(absCh1 / 2, 1) * 20;
    sc_mom += 5 + Math.min(absCh3 / 5, 1) * 20;
