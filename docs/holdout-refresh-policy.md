...
3. **Cumulative seals (belt-and-suspenders).** 20 or more distinct pre-reg seals have been filed against the current manifest. Measure by counting files in `docs/holdout-evaluations/` whose seal-date (embedded in the `<YYYY-MM-DD>-*.md` filename) is on or after the date of the most recent entry in `docs/holdout-refresh-log.md`. `scripts/autoresearch/lib/trial-ledger.ts` maintains only a GLOBAL attempt ordinal and is NOT manifest-scoped — do not use it for this trigger. If this fires before the calendar backstop, refresh early — it means research activity has been unusually dense.
...
2. **Roll holdout boundaries.** The new `holdoutStartDate` is the next trading day AFTER the old `holdoutEndDate` (both manifest boundaries are inclusive — `windowFallsInHoldout` in `scripts/autoresearch/lib/dataset-manifest.ts` uses `>=`/`<=` — so reusing the old end date as the new start would leave that day in both holdouts). The new `holdoutEndDate` is the new `dataEndDate`. Old holdout data becomes new selection data. Alternative: shift the full window forward if the new data is long enough to host a fresh 12-month holdout AND the selection window stays ≥ 5 years.
