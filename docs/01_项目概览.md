...
**目标**: 基于 TradingView `MB_DFP_Options_Scanner` 信号，锁定精确期权合约。由于跨策略分析开销极大且容易导致决策疲劳，筛选器现已专门为了与 TV 信号 1:1 对接而重构。

**前端输入**: Ticker, Direction, Target Strategy (TV推荐), Setup (TV环境)
...
**扫描流程**:
1. 交易者在 TradingView 获取明确的 `Ticker`、`Setup`（如 Pullback Buy）和 `Strategy Type`（如 Credit Put Spread）
2. 将这三个参数输入到 Option Selector
3. 获取基于该唯一策略的完整期权链数据
4. 应用硬过滤器（DTE、Strike Range、Volume、Spread）
5. 计算原始指标（Lambda、Gamma Efficiency、Theta Burn 等）
6. Z-Score 标准化并计算综合得分（0-100）
7. 按得分排序展示最佳执行结构
