...
- ✅ Watchlist + Scanner Score 强制入场计划
- ✅ Stop Loss 规则 + 视觉警告系统
- ✅ 到期日警告 + 持仓天数追踪
- ✅ 30秒快速操作 + 移动端友好
- ✅ 自动获取期权价格和Greeks数据
...
| 手动更新价格麻烦 | 自动获取期权价格（Polygon 主 / CBOE 备） |
...
└─────────────────┬────────────────────────────┘
                  ...
                  ▼
┌─────────────────────────────────────────────┐
│  期权数据源 (DATA_SOURCE 环境变量)            │
│  主: Polygon.io — 实时报价 + Greeks + IV     │
│  备: CBOE 延迟 API — 15分钟延迟，免费        │
│  Nasdaq API — 历史/财报（独立）               │
└─────────────────────────────────────────────┘
```
...
| GitHub | - | 版本控制，自动部署触发 |
| **期权数据源** |
| Polygon.io | 主 | 实时报价，Greeks + IV；仅请求所需 DTE/行权 + 1 分钟缓存；需 API Key |
| CBOE API | 备 | 15 分钟延迟，免费；Greeks 在 API 中为 0 |
...
### 3. Option Selector (靶向期权筛选)
**目标**: 基于 TradingView `MB_DFP_Options_Scanner_v3.2` 信号，锁定精确期权合约。由于跨策略分析开销极大且容易导致决策疲劳，筛选器现已专门为了与 TV 信号 1:1 对接而重构。

**前端输入**: Ticker, Direction, Target Strategy (TV推荐), Setup (TV环境), 期望参考 (根据TV Backtest)

**核心架构**: 
- **TV Scanner v3.2 (前置信号)**：多维度技术评分产生 Tiers (S/A/B/C) + Setup 识别结构建议 + Forward Expectancy Backtesting (14d/30d TP/SL 胜率模拟)。
- **OSS (Options Scoring System) (选号引擎)**:
  - **LOQ** (Long Option Quality): 买方评分系统 (v2.1 Hybrid Weights)
  - **CSQ** (Credit Spread Quality): 卖方评分系统
  - **IV Term Structure**: 4-Card Method + Sigmoid Phase Transition 波动率分析
  - **Day Trade Mode**: 自动调整杠杆/爆发力引擎权重

**扫描流程**:
1. 交易者在 TradingView 获取明确的 `Ticker`、`Setup`（如 Pullback Buy）、`Strategy Type`（如 Credit Put Spread）、以及 Score Tiers (S/A/B/C)。
2. 查看 TV 里的 **Backtesting 表格**，明确该品种该 Tier 下的 14d/30d 胜率与平均涨跌幅，并以此决定 `Stop Loss / Take Profit` 预期参数。
3. 将 Ticker、Strategy Type、Setup 参数输入到 Web App Option Selector。
4. 获取基于该唯一策略的完整期权链数据。
5. 系统自动应用硬过滤器（DTE、Strike Range、Volume、Spread）。
6. 计算原始指标并转化为综合得分（0-100）。
7. 按得分排序，筛选出与 TV 前置信号最契合的最佳期权合约。
