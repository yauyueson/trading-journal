...
> - **BCD QQQ wide** (`strategy_type='bcd'`, $2K 级)：bull call debit spread，long δ 0.50 / short δ 0.20，DTE 30-60，PT 50%，10 交易日手动触发 + maxPositions=1 flat-gate。
...
>
> DTE5 / Swing / ShortTerm 均已退役（`RETIRED_STRATEGIES`），历史持仓在 Portfolio / Stats 的 Legacy filter 下可见。手动入场使用 `BCDEntryModal` / `PMCCEntryModal`，无 cron 驱动的信号扫描。本文档部分小节仍描述旧 DTE5 流程，请以 [docs/README.md](./README.md) 和 [CLAUDE.md](../CLAUDE.md) 的 Active Strategies 章节为准。
...
│  ├─ /api/option-price.js (价格+Greeks，双数据源) │
│  ├─ /api/scan-options.js、strategy-recommend.js │
│  ├─ /api/check-alerts.js、daily-recap.js    │
│  ├─ /api/backtest-candles.js (回测K线数据)  │
│  └─ /api/earnings.js、iv-rank.js、analytics.js 等│
└─────────────────┬────────────────────────────┘
                  │
                  ▼
┌─────────────────────────────────────────────┐
...
│  ├─ positions (持仓)                         │
│  ├─ transactions (交易记录)                  │
│  ├─ position_greeks_history (Greeks历史)             │
│  ├─ ticker_iv_snapshots (IV快照)            │
│  ├─ candidate_snapshots (候选快照)           │
│  ├─ score_history (评分历史)                 │
│  └─ app_settings (应用设置)                  │
└─────────────────┬────────────────────────────┘
                  │
...
| React Router | 6.x | 声明式路由，懒加载页面 |
| React Query | 5.x | 服务端状态管理，缓存与失效 |
| **Backend** |
...
| GitHub | - | 版本控制，自动部署触发 |
| **数据源** |
| ORATS | 主 | 期权链、Greeks、IV、cores、earnings、impliedMove |
| Tiingo | 辅 | 股票日线/4H K线、历史价格 |
...
- ⚠️ 多级警告系统（危险/警告/信息）
- 📊 Greeks历史图表（IV和Delta）
- 🎯 Entry Score vs Current Score对比
- 🛑 止损触发视觉提示
...
### 3. Strategy Recommender (策略推荐)
**目标**: 基于 IV 环境和市场方向，推荐最优期权策略与合约。

**路由**: `/selector`

**功能**:
- 输入 Ticker 和方向（BULL/BEAR），系统基于 IV 环境推荐 Credit/Debit Spread 或单腿策略
- 根据 IV Regime 和 Term Structure 自动匹配策略类型
- 推荐结果可直接加入 Portfolio
- **⚠️ F1 改造后状态**: BCD + PMCC 入场改由 Signals 页面的 `BCDEntryModal` / `PMCCEntryModal` 手动触发，不经过 Strategy Recommender。该页面仍可用于手动查看一次性策略推荐或 legacy 策略分析。

**工作流**:
1. 在 Signals 页面确认 EMA34 信号（QQQ 处于均线上方）
2. 进入 Strategy Recommender，输入 QQQ + BULL 方向
3. 系统根据当前 IV 环境推荐 Bull Put Credit Spread 合约
4. 确认合约参数（DTE、Delta、Strike）后加入持仓
...
interface Position {
  ...
  strike: number;                // 行权价
  type: string;                  // 期权类型 "Call"/"Put" 或价差如 "Credit Put Spread"
  ...
  stop_price: number;            // 手动止损价
  is_paper: boolean;             // 是否为模拟交易
  strategy_type: string;         // 策略类型 (bcd/pmcc 活跃；swing/shortTerm/dte5 退役)
  legs: object[];                // 价差腿 [{strike, type, side, expiration}]
  owner: string | null;          // 持仓归属人 ('Yuchen' / 'Annie' / null)
  strategy: string;              // 期权结构类型
  exit_type: string;             // 平仓原因 (stop_loss/take_profit/expired/manual)
  tech_score: number;            // 技术面评分 (0-100)
  tech_score_source: string;     // 评分来源 (auto/manual)
  market_state: string;          // 入场时市场状态
  trade_profile: string;         // 交易风格 (day_trade/swing/position)
  iv_rank_entry: number;         // 入场时 IV Rank
  iv_regime_entry: string;       // 入场时 IV Regime
  notes: string;                 // 交易笔记
...
### 已完成（原规划项）
- [x] 回测功能（Backtest 页面 — GA 优化 + Walk-Forward + 质量门控）
- [x] 推送通知（Discord Webhook 自动提醒）
