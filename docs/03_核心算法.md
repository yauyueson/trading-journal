...
### 2. IV/RV Ratio (Volatility Risk Premium)

**目的**: 衡量期权价格相对于实际价格变动的水平。

**公式**:
```
IV/RV Ratio = IV30 / RV20
```

**判别标准**:
- **Ratio > 1.25**: 极高 VRP。期权昂贵，非常利于卖方 (Seller's Edge)。
- **Ratio 1.05 - 1.25**: 正常 VRP。期权价格合理反映波动率。
- **Ratio < 0.85**: 负 VRP。期权便宜，利于买方 (Buyer's Territory)。

**LOQ (买方) 调整**:
```typescript
const vrpAdj = Math.min((0.85 - ivRvRatio) * 4, 1.5);
// 当 Ratio < 0.85 时，得分最高可获得 +1.5 加分
```

**CSQ (卖方) 调整**:
```typescript
const vrpAdj = Math.min((ivRvRatio - 1.25) * 4, 1.5);
// 当 Ratio > 1.25 时，得分最高可获得 +1.5 加分
```
