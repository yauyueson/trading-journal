...
**公式**:
```
Average Cost = Σ(Buy Price × Buy Quantity) / Total Quantity
```
...
**公式**:
```
Current Quantity = Σ(All Transactions Quantity)
```

**代码实现**:
```typescript
function calculateCurrentQuantity(transactions: Transaction[]): number {
  return transactions.reduce((sum, tx) => sum + tx.quantity, 0);
}
```
...
**代码实现**:
```typescript
export function calculatePnL(position: Position, transactions: Transaction[]) {
  const avgCost = calculateAverageCost(transactions);
  const currentQty = calculateCurrentQuantity(transactions);
  
  // 未实现P&L
  const unrealizedPnL = position.current_price 
    ? (position.current_price - avgCost) * currentQty * 100
    : 0;
  ...
  for (const tx of transactions) {
    if (tx.quantity < 0) { // 卖出
      realizedPnL += (tx.price - avgCost) * Math.abs(tx.quantity) * 100;
    }
  }
