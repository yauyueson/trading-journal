...
**版本**: 2.2 (Volatility Risk Premium Optimized)

**v2.2 新增**:
- **IV/RV Ratio (VRP)**: 整合隐含波动率与历史波动率之比，量化波动率风险溢价。
- **Deal Breaker Filter**: 增加流动性硬约束，买卖价差率 > 15% 的期权自动归零。
- **Batch API Data Flow**: 优化持仓同步逻辑，支持批量数据获取并减少 API 调用。
...
```typescript
const LOQ_WEIGHTS = {
  lambda: 0.40,      // 杠杆率（固定权重）
  gammaEff: 0.30,    // 爆发力
  thetaBurn: -0.15,  // 时间衰减（负权重）
...
**公式** (v2.2 更新):
```
LOQ_Raw = 0.40×z(Λ) + 0.30×z(Γeff) - 0.15×z(TB) + 0.15×ΔBonus + IV_Adj - ThetaPainPenalty
...
// 3. 加权计算
const rawScore = 
  0.40 × 1.2 +      // Lambda贡献: 0.48
  0.30 × 0.8 +      // Gamma贡献: 0.24
  -0.15 × (-0.5) +  // Theta贡献: 0.075
  0.15 × 1.0 +      // Delta贡献: 0.15
  0.5;              // IV调整: 0.5 (Contango奖励)
  = 1.445

// 4. 转换为0-100分
const score = 50 + 1.445 × 12.5 = 68分
```

**结果解读**: 68分 - 一般机会，可以考虑但不是最优
...
### 2. Volatility Regime Matrix (波动率机制矩阵)

**版本**: v2.2 (Harmony Update)

**核心理念**: 
不再简单地叠加 IV Term Structure (IV30/IV90) 和 IV/RV Ratio (VRP) 的分数，而是将两者结合成一个 **4象限决策矩阵**。这种非线性逻辑能有效区分“价值机会”与“波动陷阱”。

#### 矩阵分布与决策

| 象限 | 条件 1 (Term Structure) | 条件 2 (IV/RV Ratio) | 状态 | 决策建议 | LOQ/CSQ 调整分 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1. Value Zone** | **Contango** (近期IV低) | **Low VRP** (IV < RV) | **绝对价值** | 强烈买入 (Strong Buy) | +2.5 / -2.5 |
| **2. Momentum Zone** | **Backwardation** (近期IV高) | **Low VRP** (IV < RV) | **动量扩张** | 投机买入 (Chase Move) | +1.0 / -1.0 |
| **3. Trap Zone** | **Contango** (近期IV低) | **High VRP** (IV > RV) | **价值陷阱** | 观望/避开 (Avoid Trap) | -2.0 / +2.0 |
| **4. Fear Zone** | **Backwardation** (近期IV高) | **High VRP** (IV > RV) | **极端恐慌** | 强烈卖出 (Strong Sell) | -3.0 / +3.0 |

#### 象限详解:

1.  **Value Zone (价值区)**: 
    - 期权相对于历史价格（90天）便宜，且相对于当前实际波动（20天）也便宜。
    - 这是买方最完美的安全边际。
2.  **Momentum Zone (动量区)**: 
    - 虽然近期 IV 比 90 天 IV 高（看似贵了），但实际上股价波动的速度（RV）超过了 IV 隐含的预期。
    - 属于“买贵但由于爆发力太强依然值得买”的场景。
3.  **Trap Zone (陷阱区)**: 
    - 期权看似便宜（近期 IV 低），但实际上隐含波动率依然显著高于股价实际波动。
    - 在这种平淡市场里买入期权会面临严重的时间虚耗，是买方的“价值坑”。
4.  **Fear Zone (恐慌区)**: 
    - 市场极度恐慌，期权价格相对于历史和现实都极度高昂。
    - 卖方的天堂，通过卖出高额溢价获得极高的胜率。
...
**定义**: 在计算任何分数前强制触发的退出条件。

### 1. Liquidity Guard (流动性卫士)

**规则**: 如果期权的买卖价差率 (Spread %) 过大，流动性过低，直接强制得分为 0 分。

**公式**:
```typescript
if (spreadPct > 0.15) {
  finalScore = 0;
  whyThis = "SPREAD TOO WIDE (>15%)";
}
```

**意义**: 保护交易者不进入难以平仓且滑点巨大的头寸。
...
### 1. 信用价差评分 (Credit Spread Score - CSQ+)

**核心权重**:
- **ROI (40%)**: 权利金 / 最大风险。25% ROI 达到满分。
- **POP (40%)**: 盈利概率 (1 - |Short Delta|)。
