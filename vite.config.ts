  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use('/api/backtest-iv', async (req, res) => {
        ...
        try {
          const dynamicImport = new Function('path', 'return import(path)') as (p: string) => Promise<any>;
          const handlerPath = new URL('./api/backtest-iv.js', import.meta.url).href;
          const mod = await dynamicImport(`${handlerPath}?_t=${Date.now()}`).catch(
            () => dynamicImport(handlerPath)
          );
          const handler = (mod as any).default ?? mod;
          ...
          console.error('[vite-shim] /api/backtest-iv error:', error.message);
          res.statusCode = 500;
          res.end(JSON.stringify({ error: 'Internal Server Error', message: error.message }));
        }
      });
      ...
      server.middlewares.use('/api/strategy-recommend', async (req, res) => {
        ...
        try {
          // Use new Function to create a runtime-only dynamic import.
          // esbuild cannot statically analyze imports created via new Function,
          ...
          // Build a minimal req/res shim compatible with the Vercel handler signature
          const mockReq: any = {
            method: req.method || 'GET',
            query: Object.fromEntries(url.searchParams.entries()),
            headers: req.headers,
          ...
          console.error('[vite-shim] /api/strategy-recommend error:', error.message);
          res.statusCode = 500;
          res.end(JSON.stringify({ error: 'Internal Server Error', message: error.message }));
        }
      });
